...
export default function AllTodo() {
  const allValue = useContext(TodoContext);
  const router = useRouter();

  if (!allValue) return null;

  const { todo, deleteTodo } = allValue;
    ...
    <section className="text-gray-600 body-font">
      <div className="container px-5 md:py-6 py-24 mx-auto">
        <p className="mb-3 text-gray-300 text-lg font-extrabold">
          All Todos Task We Have : {todo?.length}
        </p>
              ...
              <div className="h-full border border-gray-700 border-opacity-60 rounded-lg overflow-hidden">
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between flex-wrap ">
                    ...
                    <button onClick={() => {router.push(`/app/edit/${e.id}`)}} className="text-white pr-4 pl-4 text-center bg-blue-700 p-1 rounded inline-flex items-center md:mb-2 lg:mb-0">
                      Edit
                    </button>
                  </div>
                </div>
